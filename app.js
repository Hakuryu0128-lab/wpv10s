/* ═══════════════════════════════════════════════════════════
   WEEKY v10.0.0 — Application Logic
════════════════════════════════════════════════════════════ */
...
function save() {
  try {
    localStorage.setItem('weeky_v10', JSON.stringify({
    ...
    const el = document.getElementById('autosaveStatus');
    if (el) el.textContent = '⚠ 保存失敗（容量不足）';
  }
}
...
function load() {
  try {
    ...
    if (!raw) { ensureDefaultSchool(); return; }
...
function allClassesGrouped() {
  const multi = state.schools.length > 1;
  return state.schools.map(sc => {
    ...
    return {
      schoolName: sc.name,
      options: list.map(name => ({ value: multi ? sc.name + name : name, label: name })),
    };
...
function initActivationGate() {
  if (localStorage.getItem('weeky_v10_activated') === '1') return startApp();
  ...
  function tryActivate() {
    ...
    if (ACTIVATION_CODES.includes(code)) {
      localStorage.setItem('weeky_v10_activated', '1');
...
function lessonTileHtml(lesson) {
  ...
  const hasHw    = !!(lesson.hwPages?.length);
  const hasNote  = !!(lesson.note && lesson.note.trim());
  const clipIcon = '<svg viewBox="0 0 16 16" fill="none" width="13" height="13" aria-hidden="true"><path d="M11 5L6 10a1.5 1.5 0 002 2l5-5a3 3 0 00-4-4l-5 5a4.5 4.5 0 006 6l4-4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  return `
    ...
    <div class="tile-icons">
      ...
      ${hasPhoto ? `<span class="tile-icon tile-icon--clip" title="添付ファイル">${clipIcon}</span>` : ''}
...
function saveLessonModal() {
  ...
  const hasHw = !!(prev.hwPages?.length);
...
function renderProgressTable() {
  ...
  const names = new Set();
  (state.classes || []).forEach(c => { if (c && c.name) names.add(normClass(c.name)); });
  (state.students || []).forEach(s => { if (s.className) names.add(normClass(s.className)); });
  ...
  for (let i = 0; i < maxN; i++) {
    rows += `<tr><td class="pc-rownum">${i + 1}</td>` + perClass.map(pc => {
      ...
      return `<td class="pc-cell"><button class="pc-lesson" data-key="${escHtml(item.key)}">
        ...
        <span class="pc-lesson-date">${md}・${item.period === 'after' ? '放課後' : item.period + '限'}</span>
      </button></td>`;
    }).join('') + '</tr>';
  }
  if (maxN === 0) {
  ...
  // click a cell → jump to that lesson in the week grid
  container.querySelectorAll('.pc-lesson').forEach(btn => {
    btn.addEventListener('click', () => {
      const [dateStr] = btn.dataset.key.split('_');
      state.currentWeekStart = getWeekStart(new Date(dateStr + 'T00:00:00'));
...
function markBackupDone() {
  try { localStorage.setItem('weeky_v10_lastBackup', String(Date.now())); } catch (e) {}
  hideBackupBanner();
}
function checkBackupReminder() {
  const last = parseInt(localStorage.getItem('weeky_v10_lastBackup') || '0', 10);
...
function showBackupBanner(last) {
  ...
  banner.querySelector('#backupDismissBtn').addEventListener('click', () => {
    // snooze 1 day
    try { localStorage.setItem('weeky_v10_lastBackup', String(Date.now() - 6*86400000)); } catch(e){}
    hideBackupBanner();
  });
}
...
function needsOnboarding() {
  if (localStorage.getItem('weeky_onboarded') === '1') return false;
  ...
  if (hasData) { localStorage.setItem('weeky_onboarded', '1'); return false; }  // 既存ユーザーは対象外
  return true;
}
...
function finishOnboarding() {
  ...
  localStorage.setItem('weeky_onboarded', '1');
  save();
  const ov = document.getElementById('onboarding');
